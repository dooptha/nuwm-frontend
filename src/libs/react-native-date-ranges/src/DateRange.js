...
import {
  Text, View, TouchableOpacity, Picker,
} from 'react-native';
...
const styles = {
  ...
  headActionContainer: { // month year
    flexDirection: 'row',
    alignItems: 'center',
...
export default class DateRange extends Component {
  ...
  onDatesChange = (event) => {
    ...
    const defalutFormat = !this.props.mode || this.props.mode === 'single'
      ? 'ddd, MMM D'
      : 'MMM DD,YYYY';
  ...
  render() {
    ...
    const { customStyles = {}, startDateLabel, endDateLabel } = this.props;
    ...
    return (
      <View style={customStyles.sub}>
          ...
          {this.props.mode === 'single' && (
            <View>
              ...
              <TouchableOpacity onPress={this.selectMonthAndDate}>
                <Text
                  style={{ fontSize: 40, ...customStyles.mainText, fontWeight: 'bold' }}
                >
          ...
          {this.props.mode === 'range' && (
            <View>
              ...
              <View style={styles.dateContainer}>
                <Text style={[headerDate, customStyles.mainText]}>
                  {this.state.clearStart ? this.state.clearStart : startDateLabel}
                </Text>
                <Text style={styles.headTitleText} />
                <Text style={[headerDate, customStyles.mainText]}>
                  {this.state.clearEnd ? this.state.clearEnd : endDateLabel}
                </Text>
              </View>
            </View>
          )}
        ...
        {this.state.selectState === 'monthAndDate' && (
          <View style={[styles.calendar, customStyles.calendar, customStyles.sub]}>
            <View style={styles.headActionContainer}>
              ...
              <Text
                style={{ fontSize: 20, ...customStyles.mainText, fontWeight: 'bold' }}
              >
            ...
            <Month
              textColor={customStyles.subText.color}
        ...
        {this.state.selectState === 'year' && (
          <View
            style={[
              styles.calendar,
              { height: '75%', justifyContent: 'center', ...customStyles.sub },
            ]}
          >
            <Picker
